class Solution {
    ...
    private int eval(String exp, Map<String, Integer> parent) {
        if(exp.charAt(0) != '(') {
            if(Character.isDigit(exp.charAt(0)) || exp.charAt(0)=='-'){ //0-9或负号
                return Integer.parseInt(exp);
            }
            return parent.get(exp); //变量，我们直接从哈希map中取值即可
        }
        // create a new scope, add add all the previous values to it
        Map<String, Integer> map = new HashMap<>();
        map.putAll(parent); //将一个已有Map中的数据压入另一个Map中，且去重。
        ...
        if(exp.startsWith("(a")){ // add
            ...
            for (int i=0; i<tokens.size()-2; i+=2){
                map.put(tokens.get(i), eval(tokens.get(i+1), map)); // 如：x 2
            }
